...
fn collect(node: &Node, src: &str, out: &mut Vec<Symbol>) {
    match node.kind() {
        ...
        "interface_declaration" => {
            if let Some(sym) = make_sym(node, src, "interface") {
                out.push(sym);
            }
            return;
        }
        "type_alias_declaration" => {
            if let Some(sym) = make_sym(node, src, "type_alias") {
                out.push(sym);
            }
            return;
        }
        "enum_declaration" => {
            if let Some(sym) = make_sym(node, src, "enum") {
                out.push(sym);
            }
            return;
        }
        // Arrow function assigned to a const/let: `const foo = () => {}`
...
// Detect `const name = (...) => ...` at the top level of a declaration.
fn arrow_sym(node: &Node, src: &str) -> Option<Symbol> {
    // Must have exactly one declarator with an arrow_function value.
    let mut cur = node.walk();
